...
const log = (string: string) =>
  console.log(
    c.bgGreen(c.black(` ${appPackageJSON.name} `.toUpperCase())),
    string,
  )
...
const pnpmInstall = (cmd: string[]) =>
  execa('pnpm', ['add', ...cmd], { stdio: 'inherit' })
...
async function pkg(options: CommandOptions) {
...
async function main(_: undefined, options: CommandOptions) {
